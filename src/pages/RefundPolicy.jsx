...
export default function RefundPolicy() {
  ...
  useEffect(() => {
    const fetch = async () => {
      setLoading(true);
      try {
        const res = await api.get('/policies/refund');
  ...
  if (!policy) {
    return (
      <main className="w-full pt-20">
        <section className="py-24 bg-white text-center">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h3 className="font-display font-bold text-slate-900">Refund Policy Not Available</h3>
            <p className="text-xs text-slate-500 mt-2">We couldn't retrieve the refund policy at this time.</p>
          </div>
        </section>
      </main>
    );
  ...
  return (
    <main className="w-full pt-20">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-xs text-slate-600 leading-relaxed space-y-6 rich-text-content">
          <div>
