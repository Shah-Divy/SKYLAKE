...
export default function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex font-sans">
      ...
      <aside
        className={`hidden lg:flex flex-col justify-between shrink-0 bg-slate-900 text-slate-400 border-r border-slate-800 transition-all duration-300 ${
      ...
      >
        <div>
          ...
          <nav className="p-3.5 space-y-1 max-h-[70vh] overflow-y-auto no-scrollbar">
            {menuItems.map((item) => (
              <NavLink
                key={item.name}
                to={item.path}
        ...
        <header className="h-20 bg-white border-b border-slate-200/60 px-6 sm:px-8 flex items-center justify-between z-30 shadow-sm shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-xs text-slate-400 font-bold hidden sm:block">
              Welcome, <span className="text-slate-700 font-extrabold">{adminUser?.name || 'Admin'}</span> ({adminUser?.role})
            </div>
          </div>
                ...
                {profileOpen && (
                  ...
                  >
                    <div className="p-2 border-b border-slate-100/60 mb-2">
                      <h4 className="font-bold text-slate-900">{adminUser?.name || 'Administrator'}</h4>
